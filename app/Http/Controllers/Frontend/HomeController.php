...
class HomeController extends Controller
{
    ...
    {
        $id = Auth::user()->id;
        $jobsApplied = JobApplication::where('user_id',$id)->count();
        return view('frontend.pages.dashboard.dashboard-page',compact('jobsApplied'));
    }
    ...
    {
        $user = Auth::user();
        $job = Job::with('company','category')->where('id',$job_id)->first();
        return view('frontend.pages.dashboard.apply-job-page', compact('job','user'));
    }
    ...
    public function storeJobApplication(Request $request)
    {
        ...
        $company_id = Job::where('id',$job_id)->pluck('company_id')->first();

        $result = JobApplication::where('user_id',$user_id)->where('job_id',$job_id)->first();
        if($result){
            return redirect()->back()->with('error', 'You have already applied this job. Try another !');
        }else{
            ...
            return redirect()->route('applied.job.list')->with('success', 'Successfully placed your application !');  
        }
    ...
    {
        $user_id = Auth::user()->id;
        $jobs = JobApplication::with('user','job')->latest()->where('user_id',$user_id)->get();
        return view('frontend.pages.dashboard.applied-job-list-page',compact('jobs'));
    }

    public function appliedJobDetails($id)
    {
        $data = JobApplication::with('user','job')->where('id',$id)->first();
        return view('frontend.pages.dashboard.applied-job-detail-page',compact('data'));
    }

    public function editResumeByJob($job_id)
    {
        $id = Auth::user()->id;
        $editData = Resume::with('educations','trainings','job_experiences')->where('user_id',$id)->first();
        return view('frontend.pages.dashboard.edit-resume-by-apply-job-page', compact('editData','job_id'));
    }


    public function updateResumeByJob(Request $request, $id)
    {
        //dd($request->job_id);
        ...
        $request->validate([
            ...
            'email'       => 'required|string|email|max:50|unique:resumes,email,'.$id,
        ...
        return redirect()->route('apply.job', ['job_id' => $job_id])->with($notification);
    }

    public function createResumeByJob($job_id)
    {

        return view('frontend.pages.dashboard.create-resume-by-apply-job-page', compact('job_id'));
    }


    public function storeResumeByJob(Request $request)
    {
        //dd($request->all());
        $request->validate([
            'name'        =>'required|string|max:50',
